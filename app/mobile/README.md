...
④ `cap sync` → ④·⑤ 自动开启 `android:usesCleartextTraffic`（允许连接局域网 HTTP）→ ⑤ `cap build android` →
⑥ 定位 `app-debug.apk`，若已连手机则 `adb install -r` 自动装上。
...
`Cleartext traffic not permitted` / ATS 阻断。

- **Android 已自动处理**：一键脚本在 `cap sync` 后会自动给 `android/app/src/main/AndroidManifest.xml`
  的 `<application>` 加上 `android:usesCleartextTraffic="true"`，因此出包后**直接连局域网
  `http://` 家庭服务器即可**，无需手动改原生工程（幂等，重复构建不会重复添加）。
- **iOS 仍需手动放行**：iOS 需在 `ios/App/App/Info.plist` 加 `NSAppTransportSecurity` 例外（允许该局域网域名）。
- **更稳的做法（推荐生产）**：家庭服务器走 HTTPS（如 Cloudflare 隧道 / 反向代理加证书），App 填 `https://域名`；
  若已用 HTTPS，可在 `build.js` 注释掉 `patchAndroidManifest()` 调用以收紧安全。
